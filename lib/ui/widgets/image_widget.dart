...
class RoundNetworkImage extends StatelessWidget {
  ...
  RoundNetworkImage({required this.width, required this.height, this.url, this.radius, this.boxFit, this.strokeColor, this.strokeWidth, this.showImageError = true, this.isLocal = false});

  @override
  Widget build(BuildContext context) {
    return ClipRRect(
      borderRadius: BorderRadius.circular(radius ?? 0),
      child: (url != null && isLocal == false)
          ? CachedNetworkImage(
...
class AvatarNetworkImage extends StatefulWidget {
  ...
  final bool? isLocal;
  AvatarNetworkImage(
      {required this.width, required this.height, this.url, this.radius = 0.0, this.errorHolder, this.boxFit, this.strokeColor, this.strokeWidth, this.userName, this.textStyle, this.isLocal = false});
...
class _AvatarNetworkImageState extends State<AvatarNetworkImage> {
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: (url != null && widget.isLocal == false)
          ? CachedNetworkImage(
              cacheManager: CustomCacheManager.instance,
              fadeInDuration: const Duration(milliseconds: 300),
              fadeOutDuration: const Duration(milliseconds: 500),
  ...
  Widget _buildErrorWidget(double radius) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          gradient: LinearGradient(begin: const Alignment(1, 0), end: const Alignment(0, 1), colors: [
